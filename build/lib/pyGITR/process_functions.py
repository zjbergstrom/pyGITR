...
def getSurfaceData(ncData):
    ''' dim_dict: nSurfaces, nEnergies, nAngles
        var_dict: grossDeposition[nSurfaces], grossErosion[nSurfaces], 
            aveSpyl[nSurfaces], spylCounts[nSurfaces], surfaceNumber[nSurfaces]
    ...
    for var in var_dict.keys():
        if variable==None or var in variable:
            if len(var_dict[var].shape)==1:
                ...
                plt.plot(np.arange(0,dim_dict["nSurfaces"]), var_dict[var], 'm--d', label="{}".format(var))
                plt.ylabel("{}".format(var))
                plt.xlabel("surface number")
                plt.legend()
                plt.show()
            else:
                for surface in range(dim_dict["nSurfaces"]): #range(0,1): 
                    # print(var_dict[var][surface])
                    x = np.arange(0,dim_dict["nEnergies"],1)
                    y = np.arange(0,dim_dict["nAngles"],1)
                    c = plt.pcolormesh(var_dict[var][surface], cmap ='Greens', vmin = np.min(var_dict[var][surface]), \
                        vmax = np.max(var_dict[var][surface]), label="{}, surface #{}".format(var,surface))
                    plt.colorbar(c)
                    plt.legend()
                    plt.show()
