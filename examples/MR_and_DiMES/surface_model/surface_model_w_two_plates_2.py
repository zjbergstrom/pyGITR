...
# count_W = 0
# print("\nW have hit the following mesh elements (not necessarily surfaces)")
# for i in surfaceHit:
#     if i != -1:
#         count_W+=1
#         print(i)
...
x1,x2,x3,y1,y2,y3,z1,z2,z3,\
    area,surf,Z,a,b,c,d,in_direction,\
        plane_norm = getGeom(GeomFile)
...
for i in range(len(Energy_particles_C)):
    if surfacehit_C[i] != -1:
        ...
        Y_CW_Gamma_C_redep[surface_index] = Y_CW_Gamma_C_redep[surface_index] + \
            sr_object.Calculate_PhysicalSputteringParameters('C','W',Energy_particles_C[i])*Flux_C_local
        Y_CC_Gamma_C_redep[surface_index] = Y_CC_Gamma_C_redep[surface_index] + \
            sr_object.Calculate_PhysicalSputteringParameters('C','C',Energy_particles_C[i])*Flux_C_local
...
for i in range(len(Energy_particles_W)):
    if surfacehit_W[i] != -1:
        ...
        Y_WW_Gamma_W_redep[surface_index] = Y_WW_Gamma_W_redep[surface_index] + \
            sr_object.Calculate_PhysicalSputteringParameters('W','W',Energy_particles_W[i])*Flux_W_local
        Y_WC_Gamma_W_redep[surface_index] = Y_WC_Gamma_W_redep[surface_index] + \
            sr_object.Calculate_PhysicalSputteringParameters('W','C',Energy_particles_W[i])*Flux_W_local
